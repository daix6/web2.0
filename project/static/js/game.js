$(document).ready(function() {
	...
	function nextBg() {
		var hold = body.css("backgroundImage");
		if (body.css("backgroundImage").search("img00") != -1) {
			if ($(this).attr("id") == "A") {
				var newBg = body.css("backgroundImage").replace("00", "01");
				$("body").css("backgroundImage", newBg);
				$("#plot-text").text("你受不了这个事实，倒在了地上，卒！");
				success = true;
			} else if ($(this).attr("id") == "B") {
				var newBg = body.css("backgroundImage").replace("00", "02");
				$("body").css("backgroundImage", newBg);
				...
				var newBg = body.css("backgroundImage").replace("00", "03");
				$("body").css("backgroundImage", newBg);
		...
		} else if (body.css("backgroundImage").search("img02") != -1) {
			if ($(this).attr("id") == "A") {
				var newBg = body.css("backgroundImage").replace("02", "04");
				$("body").css("backgroundImage", newBg);
				...
				var newBg = body.css("backgroundImage").replace("02", "05");
				$("body").css("backgroundImage", newBg);
				$("#plot-text").text("来到图书馆，看见图书馆里都是人，你又不想进去了。这时，天上一道闪电劈死了你。不恋爱又不当学霸，不如去死！卒！");
				success = true;
			} else {
				var newBg = body.css("backgroundImage").replace("02", "06");
				$("body").css("backgroundImage", newBg);
		...
		} else if (body.css("backgroundImage").search("img03") != -1) {
			if ($(this).attr("id") == "A") {
				var newBg = body.css("backgroundImage").replace("03", "07");
				$("body").css("backgroundImage", newBg);
				$("#plot-text").text("希望成真，你遇到了辅导员！辅导员走上来，神情严肃地告知你，你挂科太多，已经被劝退了。再见^_^");
				success = true;
			} else if ($(this).attr("id") == "B") {
				var newBg = body.css("backgroundImage").replace("03", "08");
				$("body").css("backgroundImage", newBg);
				$("#plot-text").text("希望成真，你遇到了看穿了一切的pml！之后，你跟随pml，成为前端大神，赢娶白富美，走上人生巅峰！");
				success = true;
			} else {
				var newBg = body.css("backgroundImage").replace("03", "09");
				$("body").css("backgroundImage", newBg);
		...
		} else if (body.css("backgroundImage").search("img04") != -1) {
			if ($(this).attr("id") == "A") {
				var newBg = body.css("backgroundImage").replace("04", "10");
				$("body").css("backgroundImage", newBg);
				$("#plot-text").text("一天的劳累之后，你在三饭大快朵颐，竟然没发现自己菜里面的虫子。三天后的夜晚，你入睡，再也没有醒来……卒！");
				success = true;
			} else if ($(this).attr("id") == "B") {
				var newBg = body.css("backgroundImage").replace("04", "00");
				$("body").css("backgroundImage", newBg);
		...
		} else if (body.css("backgroundImage").search("img06") != -1) {
			if ($(this).attr("id") == "A") {
				var newBg = body.css("backgroundImage").replace("06", "10");
				$("body").css("backgroundImage", newBg);
				...
				var newBg = body.css("backgroundImage").replace("06", "11");
				$("body").css("backgroundImage", newBg);
			...
			if ($(this).attr("id") == "A") {
				var newBg = body.css("backgroundImage").replace("09", "13");
				$("body").css("backgroundImage", newBg);
				$("#plot-text").text("你捡起了校园卡，发现卡中少女十分美丽！环顾四周并无人，保安也不在，你决定出去碰碰运气，来到了图书馆，竟然真的找到少女，并拿到了微信！之后，展开了恋爱物语！");
				success = true;
			} else if ($(this).attr("id") == "B") {
				var newBg = body.css("backgroundImage").replace("09", "09");
				$("body").css("backgroundImage", newBg);
				...
				var newBg = body.css("backgroundImage").replace("09", "14");
				$("body").css("backgroundImage", newBg);
...
var i = 0;
function typewriter(showString) {
	return function() {
		var stringLength= showString.length;
		$("#plot-text").text(function(n, text) {
			return text+showString.charAt(i);
		});
		i++;
		if (i >= stringLength) {
			clearTimeout(timeID);
			...
			timeID= setTimeout(typewriter(showString),70);
		}
	}
}
